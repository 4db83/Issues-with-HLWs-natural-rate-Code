%% Median Unbiased Estimation as implemented in HLW(2017) STAGE 2. 

function [Lambdas_ols, Lambdas_chow, Stats_ols, Stats_chow, ExtraOutput] = MUE_HLW_Stage2(YY, XX, theta)
...
%  	t2 = T-t1;
	...
	%	CREATE GY = [Y(t) -a1*Y(t-1) -a2*Y(t-2) -ar/2*(r(t-1)+r(t-2)) -ag*g(t-1)] (without subtracting the intercept a0)
	GY  = YY - XX(:,1:end-1)*theta(1:end-1);
